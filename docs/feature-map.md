# WP Title Layer 1.0.0-rc.1 功能地图
...
分配改名和重排后仍稳定的数字公开 ID。1.0.0-rc.1 在此基础上把书籍式内容明确
拆为 Series／Season 范围与序文／主要文章／尾声／附录角色。统一的“篇序与结构”
页面将有序正文篇序与可选结构轨道分开管理。

## 1. 内容语义层

| 关注点 | 当前所有者 | 1.0.0-rc.1 状态 |
| --- | --- | --- |
...
- 1.0.0-rc.1 中序文、尾声与附录各自使用私有结构轨道，不占用主要文章的自动
...
1.0.0-rc.1 为文章增加独立的书籍式范围：平铺 Series 的所有角色均属全序列；
分季 Series 的主要文章必须属于一季，非主要角色可明确选择“全序列”或“单季”。
旧的 `intro`、`epilogue`、`appendix` 若无法从已有 Season 确定范围，只进入逐
Series 只读预览，不会自动写成全序列。新建的空 Series 直接获得规范结构；已有
Series 仍须由管理员逐个确认。
...
2. 覆盖平铺／多季、有序／无序四种组合，并在多季组合中同时建立全序列序文、
   单季序文、主要文章、单季尾声、全序列尾声和多篇附录；
...
5. 核对完整 Archive、单季筛选、Single 题头、上一篇／下一篇、起点、进度与
   Series Health 使用同一范围／角色解释；单季页不得混入全序列序文或尾声；
