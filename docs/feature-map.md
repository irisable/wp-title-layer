# WP Title Layer 1.0.0-rc.4 功能地图
...
分配改名和重排后仍稳定的数字公开 ID。1.0.0-rc.4 在此基础上把书籍式内容明确
拆为 Series／Season 范围与序文／主要文章／尾声／附录角色。统一的“篇序与结构”
页面将有序正文篇序与可选结构轨道分开管理。

rc.4 收口：全系列非正文按角色合并公共栏目，公开标签留在文章行；编辑器先角色后范围与季，内容分组仅主要文章生效。新增只读版本化能力供 Publisher 发布高级角色，后台数据结构保持不变。

## 1. 内容语义层

| 关注点 | 当前所有者 | 1.0.0-rc.4 状态 |
| --- | --- | --- |
...
- 1.0.0-rc.4 中序文、尾声与附录各自使用私有结构轨道，不占用主要文章的自动
...
1.0.0-rc.4 为文章增加独立的书籍式范围：平铺 Series 的所有角色均属全系列；
